...
{
    public partial class Form_33 : MetroFramework.Forms.MetroForm
    {
        //정답 배열
        readonly int[] answer = { 1,2,3,4,5,6,7,8,0};
        //숫자 보드
        int[] board = new int[9];
        //버튼 배열
        ...
        {
            for(int i = 0; i < 9; i++)//1부터 25까지 채워넣고
            {
                board[i] = i + 1;
            }
            board[8] = 0;//마지막 25번째 칸을 0으로 초기화
        ...
        void Board_Draw()
        {
            for(int i = 0; i < 9; i++)//배열의 모든 값을 받아서
            {
